...
import (
	...
	constants "ark-online-excel/common"
...
type OutputParamsDTO []dto.SheetDataGroup
// 用于存储填充字段备注值
...
// 设置单元格中已经修改的行与列的宽度
func SetColsAndRowslength(f *excelize.File,sheet dto.SheetCells,sheetname string) (error) {
	//先设置修改列的长度
	if len(sheet.Cols) != 0 {
		for k := 0;k < len(sheet.Cols);k++ {
			if sheet.Cols[k].Width != constants.NilStr {
				...
				if err != nil {
					fmt.Printf("SetColsAndRowslength ParseFloat err:%v\n",err)
					log.Fatal(err)
					return err
				}
				// fixme 这里需要重新设置一下宽度的比例

				fmt.Printf("SetColsAndRowslength SetColWidth params_bk:%v--%v\n",sheet.Cols[k].Index,sheet.Cols[k].Index)
				if err := f.SetColWidth(sheetname,sheet.Cols[k].Index,sheet.Cols[k].Index,width/5);err!=nil {
					fmt.Printf("SetColsAndRowslength SetColWidth err:%v\n",err)
					log.Fatal(err)
					return err
				}
			}
	...
	if len(sheet.Rows) != 0 {
		for k := 0;k < len(sheet.Rows);k++ {
			if sheet.Rows[k].Height != constants.NilStr {
				...
				if err != nil {
					fmt.Printf("SetColsAndRowslength width err:%v\n",err)
					return err
				}
				index ,err := strconv.Atoi(sheet.Rows[k].Index)
				if err != nil {
					fmt.Printf("SetColsAndRowslength index err:%v\n",err)
					log.Fatal(err)
					return err
				}
				if err := f.SetRowHeight(sheetname,index,width);err!=nil {
					fmt.Printf("SetColsAndRowslength height err:%v\n",err)
					log.Fatal(err)
					return err
				}
			}
...
// 循环设置每个单元格的元数据
func SetBlockStyleAndValue(f *excelize.File, data []dto.SheetDataGroup,sheetname string,username string) error {
	// 单元格样式写入对象
	opSheetCellstyles := OpSheetCellsStyle{}
	styles := []dto.StyleJson{}
	...
	for k:=0 ; k<len(data); k++ {
		...
		switch utils.IsDataKeys(data[k].Text) {
			...
			case false:
				...
				fmt.Printf("SetBlockStyleAndValue 07: %v--%v--%v \n",data[k],opSheetCellstyles,styles)
				//if err := SetCellStyle(f,data[k],opSheetCellstyles,styles);err!=nil {
				//	log.Fatal(err)
				//	return err
				//}

				if err := SetCellStyleEx(f,data[k]);err!=nil {
					log.Fatal(err)
					return err
				}

				fmt.Printf("SetBlockStyleAndValue 08 \n")
			...
			case true:
				fmt.Printf("SetBlockStyleAndValue 09 \n")
				// fixme 字段表起始值的数据
				logrus.Info("数据库字段")
...
func SetBlockStyleAndValueEx(f *excelize.File, data []dto.SheetDataGroup,sheetname string,username string) error {
	// 单元格样式写入对象
	var column_offset_map = utils.NewOffsetMap()
	var column_maxrow_map = utils.NewMaxCurrentRow()
	length := 0
...
// 设置文件样式
func SetCellStyle(f *excelize.File,sheetdata dto.SheetDataGroup,opSheetCellstyles OpSheetCellsStyle,stylesJson  []dto.StyleJson) (error) {
	...
	stylesJson = append(stylesJson,dto.StyleJson{"font",font_json})
	...
	if  sheetdata.Style.BgColor != constants.NilStr {
		...
		stylesJson = append(stylesJson,dto.StyleJson{"fill",bgcolor_json})
	}
	...
	stylesJson = append(stylesJson,dto.StyleJson{"alignment",position_json})
	...
	stylesJson = append(stylesJson,dto.StyleJson{"border",border_json})
	if len(stylesJson) != 0 {
...
func SetCellStyleEx(f *excelize.File,sheetdata dto.SheetDataGroup) (error) {
	var mergeLen = len(sheetdata.Merge)
	// 设置字体
	var fileStyle excelize.Style

	var tmpfileFont excelize.Font
	var tmpfileAlignment excelize.Alignment
	var tmpfileBorderList []excelize.Border

	//设置字体样部分
	tmpfileFont.Bold = sheetdata.Style.Font.Bold
	tmpfileFont.Italic = sheetdata.Style.Font.Italic
	tmpfileFont.Size = sheetdata.Style.Font.Size

	if  sheetdata.Style.Color != constants.NilStr {
		tmpfileFont.Color = sheetdata.Style.Color
	}
	if  sheetdata.Style.Underline == true {
		tmpfileFont.Underline =  "single"
	}
	if sheetdata.Style.Font.Name != constants.NilStr {
		tmpfileFont.Family =  sheetdata.Style.Font.Name
	}
	fileStyle.Font = &tmpfileFont


	//设置背景
	if  sheetdata.Style.BgColor != constants.NilStr {
		var tmpfileFill excelize.Fill

		tmpfileFill.Color = []string{ sheetdata.Style.BgColor}
		tmpfileFill.Pattern = 1
		tmpfileFill.Type = "pattern"

		fileStyle.Fill = tmpfileFill
	}

	// 设置字体居中
	if sheetdata.Style.Align == constants.NilStr {
		tmpfileAlignment.Horizontal = "left"
	} else {
		tmpfileAlignment.Horizontal = sheetdata.Style.Align
	}

	if  sheetdata.Style.Valign != constants.NilStr {
		// 前端中的 middle 对应的是后台的r
		if sheetdata.Style.Valign == "middle" {
			tmpfileAlignment.Vertical = "center"
		}else {
			tmpfileAlignment.Vertical = sheetdata.Style.Valign
		}
		// 默认是left
	}
	...
	tmpfileAlignment.WrapText = sheetdata.Style.TextWrap
	fileStyle.Alignment = &tmpfileAlignment


	//设置边框
	if len(sheetdata.Style.Border.Bottom)!=0 {
		tmpfileBorderList = append(tmpfileBorderList,excelize.Border {
			Type:  "bottom",
			Color:  sheetdata.Style.Border.Bottom[1],
			Style: 1,
		} )
	...
	if len(tmpfileBorderList) > 0 {
		fileStyle.Border = tmpfileBorderList
	}

	style, err := f.NewStyle(&fileStyle)
	if err != nil {
		log.Fatal(err)
		return err
	}
...
func SetCellsValue(f *excelize.File,sheetdata dto.SheetDataGroup,username string) (error) {
	//写入文本
...
// 填充字段真实值
func FillDataValue(f *excelize.File,column_offset_map,column_max_map map[string]int,outputdata dto.SheetDataGroup) (int,error){
	// 获取这个字段名
...
// 写入数据流
func doWrite(f *excelize.File,column_offset_map,column_max_map map[string]int,k int, obj interface{},param string,data dto.SheetDataGroup) (error) {
	// 返回的结果集
	var result string
	...
	for index:=0;index<k;index++{
		...
		if err = SetCellStyleEx(f,data);err!=nil {
			log.Fatal(err)
			return err
		}
